...
以下三件的共通點是:**資料是原作者的,但「條件」或「位置」是移植時決定的**。
遊戲本身可玩、可全破、可收滿成就,這三件不影響進度,只影響對原作的忠實度。
要問的話直接貼下面三段即可,不必問「這樣對不對」那種大哉問。

**1. 無主長路(masterless_road)的判定條件是什麼?**
移植版原本寫成「第十一章兩個選項之外的 fallback」,但那條路永遠走不到
(`finalChoice` 必定先寫入 `final11`),導致該結局與「天地四卷」成就都拿不到。
現在改成:第十一章選四鑰定衡 → 四鑰守衡;選萬手共衡則看第十章的對白 ——
「交給蘇檀與各地量測者盲驗」→ 公議新尺,「與裴無咎逐筆比對十代改值」→ 無主長路。
依據是兩篇結局正文各自只出現對方沒有的關鍵詞(盲測 / 裴無咎),不是原作行為。

**2. 破局答題的「現場結果」與「劇情代價」原版演在哪裡?**
`battle_beats[].action` 與 `.failure` 在移植版裡一直沒有渲染路徑(等於沒演)。
現在做成左右雙欄:左邊算式、右邊現場結果(答對)或劇情代價(答錯)。位置是移植時決定的。

**3. `battle_beats[].thread` 與 `milestones[].thread` 原版顯示在哪裡?**
同樣一直沒有渲染路徑。現在分別放在「通關畫面的本章脈絡」與「格物卷的天理分頁」。

**不需要問的**:好感增減、封印條件、成就條件、結局判定門檻 —— 那些都在 `game.json`
是反編出來的原作資料,已驗過內部一致且 8 結局 / 30 成就全部可達。

## 十、其他未決

- **新的對白節奏未經真人試玩**。收證據後的劇情改用立繪對話框演,等於每章多出約六次
  對話中斷。腳本驗過會演、演對內容、演完收乾淨,但**節奏好不好只有玩過才知道**。
  若覺得太碎,可改成只在里程碑(第 2、4 項證據)演,或把旁白留在面板、只讓角色台詞出來。
